...
| Symbolic calculus | `Symbolic` | expression parser (12 elementary functions incl. inverse-trig/hyperbolic), symbolic differentiation, algebraic simplification with like-term collection, elementary integration (incl. by-parts and arctan/arcsin forms), Taylor series, `expand`/`substitute`, polynomial `solve`/`factor`, L'Hopital `limit`, LaTeX round-trip (`toLatex`/`fromLatex`) |
